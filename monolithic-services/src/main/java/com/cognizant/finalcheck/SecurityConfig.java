...
import com.cognizant.finalcheck.controller.UserController;
import com.cognizant.finalcheck.security.AppUserDetailsService;
import com.cognizant.finalcheck.security.JwtAuthorizationFilter;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity httpSecurity) throws Exception {
		...
		httpSecurity.csrf().disable().httpBasic().and().authorizeRequests().antMatchers("/users").permitAll()
				.antMatchers("/movies").permitAll().antMatchers("/authenticate").permitAll().anyRequest()
